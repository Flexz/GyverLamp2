...
// 6х8, каждый байт - столбец, младшими разрядами вверх
...
uint8_t font6x8_getFontIndex(char asciiCode)
{
...
void font6x8_drawChar(int x, int y, char sym, CRGB color, float blendFactor)
{
  const uint8_t *syms = font6x8[font6x8_getFontIndex(sym)];
  for(int c = 0; c < 5; c++)
  {
    for(int r = 0; r < 8; r++)
    {
...
void Font6x8::DrawChar(int x, int y, char sym, CRGB color, float blendFactor)
{
    font6x8_drawChar(x, y, sym, color, blendFactor);
}
