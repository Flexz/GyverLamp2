...
inline byte scaleFF(byte x, byte b) {
  return ((uint16_t)x * (b + 1)) >> 8;
}
inline int mapFF(byte x, byte min, byte max) {
  return (((max - min) * x + (min << 8) + 1) >> 8);
}

const static char OTAhost[] = "http://ota.alexgyver.ru/";
const static char *OTAfile[] = {
  "GL2_latest.bin",
  "com_600.bin",
  "com_1200.bin",
  "esp1_600.bin",
